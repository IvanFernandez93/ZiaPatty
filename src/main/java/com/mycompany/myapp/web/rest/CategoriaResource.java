...
public class CategoriaResource {
    ...
    public ResponseEntity<CategoriaDTO> createCategoria(@RequestBody CategoriaDTO categoriaDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/categorias/" + result.getIdCategoria()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getIdCategoria().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /categorias/:idCategoria} : Updates an existing categoria.
     *
     * @param idCategoria the id of the categoriaDTO to save.
    ...
    public ResponseEntity<CategoriaDTO> updateCategoria(
        @PathVariable(value = "idCategoria", required = false) final Long idCategoria,
        @RequestBody CategoriaDTO categoriaDTO
    ) throws URISyntaxException {
        log.debug("REST request to update Categoria : {}, {}", idCategoria, categoriaDTO);
        if (categoriaDTO.getIdCategoria() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idCategoria, categoriaDTO.getIdCategoria())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, categoriaDTO.getIdCategoria().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /categorias/:idCategoria} : Partial updates given fields of an existing categoria, field will ignore if it is null
     *
     * @param idCategoria the id of the categoriaDTO to save.
     * @param categoriaDTO the categoriaDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated categoriaDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/categorias/{idCategoria}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<CategoriaDTO> partialUpdateCategoria(
        @PathVariable(value = "idCategoria", required = false) final Long idCategoria,
        @RequestBody CategoriaDTO categoriaDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update Categoria partially : {}, {}", idCategoria, categoriaDTO);
        if (categoriaDTO.getIdCategoria() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idCategoria, categoriaDTO.getIdCategoria())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, categoriaDTO.getIdCategoria().toString())
        );
    }
