...
export class CategoriaComponent implements OnInit {
  ...
  trackIdCategoria = (_index: number, item: ICategoria): number => this.categoriaService.getCategoriaIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: ICategoria[] | null): ICategoria[] {
    ...
    if (data) {
      for (const d of data) {
        if (categoriasNew.map(op => op.idCategoria).indexOf(d.idCategoria) === -1) {
